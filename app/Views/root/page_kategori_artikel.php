                            ...
                            <div class="card card-custom">
                                <div class="card-header">
                                    <div class="card-title">
                                        ...
                                        <h3 class="card-label">Seluruh Data Kategori Artikel</h3>
                                    </div>
                                                    ...
                                                    <td>
                                                        <div class="dropdown dropdown-inline mr-4">
                                                            ...
                                                            <div class="dropdown-menu">
                                                                <a class="dropdown-item" href="javascript:;" onclick="editData('<?= encrypt_url($row->kategori_id) ?>')">Edit Details</a>
                                                                <a onclick="deleteData('<?= encrypt_url($row->kategori_id) ?>')" class='dropdown-item text-danger' href='javascript:;'>Hapus Data</a>
                                                            </div>
                                                        </div>
                ...
                <form class="form" id="form_edit" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="form-group row">
                            <label class="col-xl-3 col-lg-3 col-form-label">Kategori</label>
                            <div class="col-lg-9 col-xl-9">
                                <input type="text" name="kategoriedit" id="kategoriedit" class="form-control" required placeholder="Input kategori" autocomplete="off">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        ...
        $("#form_add").submit(function(e) {
            ...
            $.ajax({
                ...
                processData: false,
                type: "POST",
                url: "<?= base_url('/crud/add-kategori-artikel') ?>",
                dataType: 'json',
                data: fdata,
        ...
        function editData(id) {
            $.ajax({
                type: "GET",
                url: "<?= base_url() ?>/crud/get-detail-kategori-artikel",
                dataType: 'json',
                data: {
                    id: id
                },
                success: function(result) {
                    if (result.status == "1") {
                        var data = result.kategori[0];
                        console.log(data);
                        $("#idedit").val(id);
                        $("#kategoriedit").val(data.kategori_nama);
                        $('#modal_edit').modal({
                            backdrop: 'static',
                            keyboard: true,
                            show: true
        ...
        $("#form_edit").submit(function(e) {
            ...
            $.ajax({
                ...
                processData: false,
                type: "POST",
                url: "<?= base_url('/crud/edit-kategori-artikel') ?>",
                dataType: 'json',
                data: fdata,
                success: function(result) {
                    setTimeout(function() {
                    ...
                    if (result.status == "1") {
                        ...
                        swal.fire("Failed!", "Gagal perbarui data !", "error");
                        $("#submiteditlogin").prop('enabled', true);
                    } else if (result.status == "3") {
                        swal.fire("Failed!", "Data tahun sudah ada !", "error");
                        $("#submiteditlogin").prop('enabled', true);
                    } else {
                        swal.fire('Error', result.status, 'error');
                        $("#submiteditlogin").prop('enabled', true);
        ...
        function deleteData(id) {
            swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "<?= base_url() ?>/crud/delete-kategori-artikel",
                        dataType: 'json',
                        data: {
                            id: id
                        },
