...
<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable page-loading">
                                        ...
                                        <div class="card card-custom">
                                            <div class="card-header">
                                                <div class="card-title">
                                                    ...
                                                    <h3 class="card-label">Artikel</h3>
                                                </div>
                                            </div>
                                            <div class="card-body">
                                                <div class="form-group">
                                                    <label>Judul</label>
                                                    <input type="text" name="juduladd" id="juduladd" class="form-control" value="<?= $artikel[0]->artikel_judul ?>" required placeholder="Input judul artikel" autocomplete="off">
                                                </div>
                                                <div class="form-group">
                                                    <label>Konten</label>
                                                    <div class="kontenadd" id="kontenadd" style="height: 500px; width:100%"><?= html_entity_decode($artikel[0]->artikel_konten) ?></div>
                                            ...
                                            <div class="card-body">
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group input-group-solid">
                                                        ...
                                                        <input type="text" class="form-control form-control-solid" value="<?= $artikel[0]->autor ?>" readonly />
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label>URL Artikel</label>
                                                    <input type="text" id="urladd" name="urladd" value="<?= $artikel[0]->artikel_url ?>" class="form-control form-control-solid" readonly>
                                                </div>
                                                <div class="form-group">
                                                    <label>Tanggal</label>
                                                    <div class="input-group input-group-solid">
                                                        ...
                                                        <input type="text" class="form-control form-control-solid" value="<?= tgl_indo(date('Y-m-d', strtotime($artikel[0]->artikel_create_time))) ?>" readonly />
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control selectpicker" data-size="7" data-live-search="true" name="kategoriadd" id="kategoriadd" required>
                                                        ...
                                                        ?>
                                                            <option value="<?= encrypt_url($row->kategori_id) ?>" <?php if ($row->kategori_id == $artikel[0]->artikel_kategori) {
                                                                                                                        echo "selected";
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control selectpicker" data-size="7" data-live-search="true" name="aksesadd" id="aksesadd" required>
                                                        ...
                                                        <option value="Draft" <?php if ($artikel[0]->artikel_akses == 'Draft') {
                                                                                    echo "selected";
                                                                                } ?>>Draft</option>
                                                        <option value="Published" <?php if ($artikel[0]->artikel_akses == 'Published') {
                                                                                        echo "selected";
                                                                                    } ?>>Published</option>
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                                    ...
                                                    <img src="<?= base_url($artikel[0]->artikel_gambar) ?>" alt="" width="80%">
                                                    <div class="custom-file mt-2">
                                                        <input type="file" class="custom-file-input" id='fotoadd' name='fotoadd' accept="" />
    ...
    <script>
        ...
        function getImageItem() {
            $.ajax({
                type: "GET",
                url: "<?= base_url() ?>/crud/get-image-artikel",
                dataType: 'json',
                data: "id=" + '<?= encrypt_url($artikel[0]->artikel_id) ?>',
                success: function(result) {
                    if (result.status == "1") {
                        var imgHTML = "";
                        $.each(result.image, function(key, value) {
                            imgHTML += value.image_item;
                        });
                        $('#img_item').html(imgHTML);
                        $('#modal_edit').modal({
                            backdrop: 'static',
                            keyboard: true,
                            show: true
        ...
        function deleteImage(image, id) {
            swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "<?= base_url() ?>/crud/delete-image-artikel-by-id",
                        dataType: 'json',
                        data: {
                            id: image,
                        },
        ...
        $("#form_add").submit(function(e) {
            ...
            fdata.append('id', '<?= encrypt_url($artikel[0]->artikel_id) ?>');
            ...
            fdata.append('rand', '<?= $artikel[0]->artikel_img_kode ?>');
            fdata.append('token', '<?= $token_edit ?>');
            ...
            $.ajax({
                ...
                processData: false,
                type: "POST",
                url: "<?= base_url('/crud/edit-artikel') ?>",
                dataType: 'json',
                data: fdata,
                success: function(result) {
                    setTimeout(function() {
                    ...
                    if (result.status == "1") {
                        ...
                        swal.fire("Failed!", "Gagal memperbarui data!!", "error");
        ...
        var KTDropzoneDemo = function() {
            var demo3 = function() {
                ...
                myDropzone5.on("sending", function(file, xhr, formData) {
                    formData.append("artikel_id", "<?= encrypt_url($artikel[0]->artikel_id) ?>");
                    formData.append("rand", "<?= $artikel[0]->artikel_img_kode ?>");
                    formData.append("action", "edit");
                    formData.append("token", '<?= $token_edit ?>');
                ...
                myDropzone5.on("removedfile", function(file) {
                    ...
                    $.ajax({
                        ...
                        data: {
                            name: fileName,
                            rand: '<?= $artikel[0]->artikel_img_kode ?>',
                            id: '<?= encrypt_url($artikel[0]->artikel_img_kode) ?>'
                        },
