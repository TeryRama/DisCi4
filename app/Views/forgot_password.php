...
<head>
	<base href="/">
	<meta charset="utf-8" />
	<title>Lupa Password | e-Sembako Plus Rohul</title>
	<meta name="description" content="Lupa password e-Sembako Plus Rohul" />
			...
			<div class="login-content flex-row-fluid d-flex flex-column justify-content-center position-relative overflow-hidden p-7 mx-auto">
				<div class="d-flex flex-column-fluid flex-center">
					<div class="login-form login-signin">
						<form class="form" id="form_forgot">
							...
							<div class="form-group">
								...
								<input class="form-control form-control-solid h-auto py-6 px-6 rounded-lg" placeholder="input username atau email" type="text" id="username" autofocus name="username" autocomplete="off" required />
								<span class="text-muted">Silahkan input username atau email anda, link untuk memperbarui kata sandi akan dikirim secara otomatis ke email yang terdaftar</span>
							</div>
							<div class="pb-lg-0 pb-5">
								<button type="submit" class="btn btn-primary font-weight-bolder font-size-h6 px-8 py-4 my-3 mr-3">Submit</button>
								<a href="<?= base_url('/login') ?>" class="btn btn-secondary font-weight-bolder font-size-h6 px-8 py-4 my-3 mr-3">Batal</a>
							</div>
						</form>
					</div>
	...
	<script>
		$("#form_forgot").submit(function(e) {
			...
			$.ajax({
				...
				processData: false,
				type: "POST",
				url: "<?= base_url('/request-forgot-password') ?>",
				dataType: 'json',
				data: fdata,
				success: function(result) {
					...
					if (result.status == "1") {
						swal.fire({
							icon: "success",
							title: "Sukses, silahkan periksa email anda",
						...
						swal.fire({
							icon: "error",
							title: "Username atau email yang anda masukkan tidak terdaftar",
							showConfirmButton: false,
							timer: 1000
						});
						...
						swal.fire("Failed!", "Gagal mengirim email, coba lagi!", "error");
						setTimeout(function() {
							location.reload();
						}, 900);
					} else {
