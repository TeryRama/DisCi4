...
class Transaksi_stok_model extends Model
{
    protected $table = 'tb_transaksi_stok';
    protected $primaryKey = 'transaksi_id';

    public function get_transaksi_by_distributor_barang_date($distributor, $barang, $date)
    {
        return $this->db
            ->table('tb_transaksi_stok')
            ->select('transaksi_stok, transaksi_id, transaksi_catatan, transaksi_ketahanan, transaksi_pemasok')
    ...
    public function get_all_transaksi_by_filter($limit, $awal, $akhir, $keyword)
    {
        $query = $this->db
            ->table('tb_transaksi_stok')
            ->select('transaksi_id, transaksi_tanggal,transaksi_barang, transaksi_distributor, transaksi_stok, transaksi_catatan, komoditas_nama, distributor_nama, komoditas_foto, komoditas_satuan')
            ->join('tb_komoditas_stok', 'tb_transaksi_stok.transaksi_barang = tb_komoditas_stok.komoditas_id')
            ->join('tb_distributor_barang_penting', 'tb_transaksi_stok.transaksi_distributor = tb_distributor_barang_penting.distributor_id')
    ...
    public function count_transaksi_by_filter($awal, $akhir, $filter)
    {
        ...
        if ($filter != "") {
            $query = $query->like('komoditas_nama', $filter, 'both');
