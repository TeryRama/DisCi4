...
class Root extends BaseController
{
    ...
    public function dashboard()
    {
        ...
        return view('root/page_dashboard', $data);
    }
    ...
    public function kategori_artikel()
    {
        ...
        return view('root/page_kategori_artikel', $data);
    }
    ...
    public function artikel()
    {
        ...
        return view('root/page_artikel', $data);
    ...
    public function add_artikel()
    {
        ...
        return view('root/page_artikel_add', $data);
    }

    public function page_management()
    {
        $data['title'] = "Management Page";
        $data['page_class'] = "management_page";
        $data['sub_panel'] = "";
        $data['page_management'] = $this->PMM->get_all_data();
        return view('root/page_management_page', $data);
    }
    ...
    public function edit_artikel()
    {
        ...
        return view('root/page_artikel_edit', $data);
        // echo json_encode($data);
    }
    ...
    public function laporan_pangan()
    {
        ...
        // echo json_encode($data);
        return view('root/page_laporan_komoditas', $data);
    }
    ...
    public function tes()
    {
        ...
        // arsort($values);
        // $popular = array_slice(array_keys($values), 0, true);

        // echo json_encode($popular);
        $array = [121000, 120000, 118000];

        $data['sum'] = count($array);
        $data['count'] = array_count_values($array);
        $data['uniq'] = array_unique($array);
        $data['sum_uniq'] = count($data['uniq']);

        $counts = array_count_values($array);
        $data['counts'] = array_slice($counts, 0, true);
        $data['max'] = $data['counts'][0];

        // $data['2000'] = $counts[2000];
        $data['ha'] = $counts;
        ...
        if ($status == 1) {
            $values = array_count_values($array);
            arsort($values);
            $populer = array_slice(array_keys($values), 0, true);
            $data['hasil'] = $populer[0];
        } else {
            $data['hasil'] = round(array_sum($array) / count($array));
        }
    ...
    public function laporan_pangan_tanggal()
    {
        ...
        // echo json_encode($data);
        return view('root/page_laporan_komoditas_tanggal', $data);
    }
    ...
    public function laporan_penting()
    {
        ...
        // echo json_encode($data);
        return view('root/page_laporan_penting', $data);
    }
    ...
    public function laporan_lainnya()
    {
        ...
        // echo json_encode($data);
        return view('root/page_laporan_lainnya', $data);
    }
    ...
    public function laporan_stok()
    {
        ...
        $data['total'] = $total;
        // echo json_encode($data);
        return view('root/page_laporan_stok', $data);
    }
    ...
    public function log()
    {
        ...
        $data['sub_panel'] = "";
        $data['page_management'] = $this->PMM->get_all_data();
        $data['log_100'] = $this->LM->get_100_log_all();
        return view('root/page_log', $data);
    }

    public function pengguna()
    {
        $data['title'] = "Kelola Pengguna";
        $data['page_class'] = "pengguna";
        $data['sub_panel'] = "";
        $data['page_management'] = $this->PMM->get_all_data();
        $data['log_100'] = $this->LM->get_100_log_all();

        $data['user'] = $this->UM->get_user('all');
        $data['role'] = $this->ULM->get_all_user_level();
        $data['pasar'] = $this->PM->get_pasar('pasar_id, pasar_nama');

        foreach ($data['user'] as $row) {
            if ($row->user_role == '3') {
                $pasar = $this->PM->select('pasar_nama')->where('pasar_id', $row->user_fk)->get()->getResult();
                $row->pasar = $pasar[0]->pasar_nama;
            }
        }

        return view('root/page_pengguna', $data);
    }
    ...
    public function kritik()
    {
        ...
        return view('root/page_kritik', $data);
    }
}
